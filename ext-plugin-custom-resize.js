var jsPsychCustomResize = (function (jspsych) {
  'use strict';
  ...
  const info = {
      ...
      parameters: {
          ...
          resize_document: {
            ...
            pretty_name: 'Resize document',
            default:  true,
            description: 'Whether to resize the document'
          }
      },
  };
  /**
   ...
   * 
   * @author Steve Chao
   * @see {@link https://www.jspsych.org/plugins/jspsych-custom-resize/ custom-resize plugin documentation on jspsych.org}
   */
  class CustomResizePlugin {
      constructor(jsPsych) {
      ...
      trial(display_element, trial) {
          ...
          var html = '<div id="jspsych-custom-resize-div" style="border: 2px solid steelblue; height: ' +
              start_div_height +
              "px; width:" +
              start_div_width +
              'px; margin: 7px auto; background-color: lightsteelblue; position: relative;">';
          html +=
              '<div id="jspsych-custom-resize-handle" style="cursor: nwse-custom-resize; background-color: steelblue; width: 10px; height: 10px; border: 2px solid lightsteelblue; position: absolute; bottom: 0; right: 0;"></div>';
          html += "</div>";
          if (trial.prompt !== null) {
              html += trial.prompt;
          }
          html += '<a class="jspsych-btn" id="jspsych-custom-resize-btn">' + trial.button_label + "</a>";
          // render
          display_element.innerHTML = html;
          // function to end trial
          const end_trial = () => {
              // clear document event listeners
              document.removeEventListener("mousemove", customresizeevent);
          ...
          document.getElementById("jspsych-custom-resize-btn").addEventListener("click", () => {
              scale();
              end_trial();
          });
          var dragging = false;
          ...
          display_element
              .querySelector("#jspsych-custom-resize-handle")
              .addEventListener("mousedown", mousedownevent);
          var mouseupevent = (e) => {
              dragging = false;
          };
          document.addEventListener("mouseup", mouseupevent);
          var scale_div = display_element.querySelector("#jspsych-custom-resize-div");
          var customresizeevent = (e) => {
              if (dragging) {
          ...
          document.addEventListener("mousemove", customresizeevent);
          // scales the stimulus
          var scale_factor;
          ...
          function scale() {
              ...
              if (trial.resize_document) {
                document.getElementById("jspsych-content").style.transform = "scale(" + scale_factor + ")";
              }
          }
      }
  }
  CustomResizePlugin.info = info;

  return CustomResizePlugin;
