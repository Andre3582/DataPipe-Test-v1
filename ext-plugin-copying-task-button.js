/**
 * jspsych-copying-task-button
 * version 0.2
 * 
 * Andre Sahakian (modified from Chris Jungerius (modified from Josh de Leeuw))
 *
 * a jsPsych plugin for displaying a copying task where the model and workspace are revealed by holding a button.
 *
 * documentation: docs.jspsych.org
 * 
 * DONE:
 *  - make bars same length/duration. Idea: put the same length bar in the middle of the red area
 *  - ensure that after error, removing the error cover is the only option.
 *  - ensure that when model is visible, the workspace and resources are not
 ...
 **/
 ...
 var jsPsychCopyingTaskButton = (function (jspsych) {
    "use strict";
  
    const info = {
      name: 'copying-task-button',
      description: '',
      parameters: {
        items: {
        ...
        sampling_delay: {
          ...
          array: false,
          pretty_name: 'Delay to reveal the model',
          default: 0,
          description: 'The duration in ms to hold a button to reveal the model.'
        },
        error_delay: {
          type: jspsych.ParameterType.INT,
          array: false,
          pretty_name: 'Delay to reveal the workspace & resources.',
          default: 0,
          description: 'The duration in ms to hold a button to reveal the workspace & resources.'
        },
        enable_delays: {
          type: jspsych.ParameterType.BOOL,
          array: false,
          pretty_name: 'Enable the delays.',
          default: true,
          description: 'Whether to enable the delays by covering the task.'
        },
        item_file_type:{
          type: jspsych.ParameterType.HTML_STRING,
          array: false,
          ...
          default: 'img',
          description: "What type of file the item is. Can be 'img' (i.e. image files like .png, .jpg etcetera) or 'svg_path') "
        },
        n_model_items: {
          type: jspsych.ParameterType.INT,
        ...
        rsc_grid_dims: {
          type: jspsych.ParameterType.INT,
          pretty_name: 'Resource gird dimensions',
          default: [4,4],
          description: 'Dimensions of the resource grid in format: [ rows, columns ].'
        },
        mdl_grid_dims: {
          type: jspsych.ParameterType.INT,
          pretty_name: 'Model (& workspace) gird dimensions',
          default: [4,4],
          description: 'Dimensions of the model (& workspace) grid in format: [ rows, columns ].'
        },
        grid_colors: {
          type: jspsych.ParameterType.HTML_STRING,
        ...
        grid_offset: {
          ...
          description: 'Offset the location of the grids (center of the grid). X and Y are in proportions of the canvas width & height.'
        },
        mid_border_specs: {
          type: jspsych.ParameterType.INT,
        ...
        disableRightClick:{
          type: jspsych.ParameterType.BOOL,
          pretty_name: 'Disable Right Click',
          default: true,
          description: 'Whether right clicks open a menu.' 
        },
        twenty_svg_paths:{
          type: jspsych.ParameterType.HTML_STRING,
    ...
    class CopyingTaskButtonPlugin {
      ...
      trial(display_element, trial) {
        ...
        // create the html 
        var html = '<div id="jspsych-canvas-copying-task-button">' 
        ...
        //   html += '<img src="' + trial.items[i] + '" id="' + trial.items[i] + '">'
        // }
        ...
        // set the top and left positions of grids on the canvas
            // model grid
        var mdl_grid_top  = canvas_height * trial.grid_offset['model_y']   - (trial.mdl_grid_dims[0] * grid_box_size) * 0.5; // vertically centered
        var mdl_grid_left = mid_border_left * trial.grid_offset['model_x'] - (trial.mdl_grid_dims[1] * grid_box_size) * 0.5; // horizontally centered between the mid left border & left canvas edge

            // resource grid
        var rsc_grid_top  = canvas_height * trial.grid_offset['resource_y']  - (trial.rsc_grid_dims[0] * grid_box_size) * 0.5; // vertically centered
        var rsc_grid_left = canvas_width  * trial.grid_offset['resource_x']; // left edge of grid at at 79% canvas width

            // workspace grid
        var wsp_grid_top  = canvas_height * 0.5  - (trial.mdl_grid_dims[0] * grid_box_size) * 0.5; // vertically centered
        var wsp_grid_left = mid_border_right + (rsc_grid_left - mid_border_right) * 0.5  - (trial.mdl_grid_dims[1] * grid_box_size) * 0.5; // horizontally centered between the mid right border & left resource grid edge



        // make an array for each grid with (row * col) number of elements
        var n_model_grid_boxes = trial.mdl_grid_dims[0] * trial.mdl_grid_dims[1];
        var n_resource_grid_boxes = trial.rsc_grid_dims[0] * trial.rsc_grid_dims[1];

        var model_grid     = new Array(n_model_grid_boxes); 
        var workspace_grid = new Array(n_model_grid_boxes);
        var resource_grid  = new Array(n_resource_grid_boxes);

            // fill the MODEL & WORKSPACE grid (arrays) with box objects and values
        for (var row=0; row<trial.mdl_grid_dims[0]; row++) {
          for (var col=0; col<trial.mdl_grid_dims[1]; col++) {
    
            var box_index = getBoxIndex(row, col, trial.mdl_grid_dims[1])
        ...
        for (var row=0; row<trial.rsc_grid_dims[0]; row++) {
          for (var col=0; col<trial.rsc_grid_dims[1]; col++) {
    
            var box_index = getBoxIndex(row, col, trial.rsc_grid_dims[1])
        ...
        for (var rep=0; rep<trial.n_model_items+1; rep++) {
          ...
          resource_grid.forEach( function(box) {
            ...
            if(trial.item_file_type == 'svg_path'){
              CreateItemFromSVG(box, options_object)
            }
        ...
        var selected_model_items = this.jsPsych.randomization.sampleWithReplacement(trial.items, trial.n_model_items);
    
          // select unique locations from the grid topleft positions (sample WITHOUT repl.)
        var selected_model_boxes = this.jsPsych.randomization.sampleWithoutReplacement(model_grid, trial.n_model_items);
        ...
        for (var i=0; i < selected_model_items.length; i++)  {
          ...
          if(trial.item_file_type == 'svg_path'){
            CreateItemFromSVG(selected_model_boxes[i], options_object)
          }
        };
        ...
        /////////////////////////
        // Add explaining text
        var mdl_text = new fabric.Text('The *example*', { 
          left: mdl_grid_left + (trial.mdl_grid_dims[0] * grid_box_size) * 0.5,
          originX: 'center',          
          top: canvas_height * 0.87, 
        ...
        var wsp_text = new fabric.Text('Recreate the *example* here\nwith the *puzzle pieces*', { 
          left: wsp_grid_left + (trial.mdl_grid_dims[0] * grid_box_size) * 0.5,
          originX: 'center',          
          top: canvas_height * 0.87, 
          fontSize: (canvas_width/65),
          fontFamily: 'Arial',
        ...
        var rcs_text = new fabric.Text('The *puzzle pieces*', { 
          left: rsc_grid_left + (trial.rsc_grid_dims[0] * grid_box_size) * 0.5,
          originX: 'center',          
          top: canvas_height * 0.87, 
          fontSize: (canvas_width/65),
          fontFamily: 'Arial',
        ...
        ///////////////////////////////////////////////////////////////////////
        /* SECTION 3: Create covers for model and workspace*/
        ...
        if (trial.enable_delays){



            // Model cover
          var model_cover = new fabric.Rect({
            fill: 'rgb(128,128,128)',
            top: mdl_grid_top,
            left: mdl_grid_left,
            width: trial.mdl_grid_dims[1] * grid_box_size, 
            height: trial.mdl_grid_dims[0] * grid_box_size, 
          })
          

          var mdl_cover_text = new fabric.Text('Click and hold here\nto uncover the example.', { 
            left: mdl_grid_left + model_cover.width/2,
            originX: 'center',
            top: mdl_grid_top*1.17, 
            fontSize: (canvas_width/65),
            fontFamily: 'Arial',
            textAlign: 'center' 
          });

          var model_cover = new fabric.Group([model_cover, mdl_cover_text])
          model_cover.set({
            hasControls: false,
            selectable: false,
          })
          ...
          model_cover.isLocked = true; // track whether model cover is locked or open
        
          var model_timeout;
          var model_timeout_start_timestamp;

          canvas.add(model_cover);


          var rcs_part_of_mdl_cover = new fabric.Rect({
            fill: 'rgb(128,128,128)',
            top: rsc_grid_top,
            left: rsc_grid_left,
            width: trial.rsc_grid_dims[1] * grid_box_size, 
            height: trial.rsc_grid_dims[0] * grid_box_size, 
          })   
          
          var wsp_part_of_mdl_cover = new fabric.Rect({
            fill: 'rgb(128,128,128)',
            top: wsp_grid_top,
            left: wsp_grid_left, 
            width: trial.mdl_grid_dims[1] * grid_box_size, 
          ...
          var workspace_cover_during_model_view = new fabric.Group([rcs_part_of_mdl_cover, wsp_part_of_mdl_cover])

          var init_workspace_cover_during_model_view_left = workspace_cover_during_model_view.left;

          workspace_cover_during_model_view.set({
            left: canvas_width,
            hasControls: false,
            selectable: false,})

          workspace_cover_during_model_view.setCoords()

          canvas.add(workspace_cover_during_model_view)  


          // Add animations (model cover)
          var init_model_cover_bar_left = model_cover.left + model_cover.width * 0.1;
          var init_model_cover_bar_width = model_cover.width * 0.8;
          var init_model_cover_bar_height = model_cover.height * 0.2;
          var init_model_cover_bar_top = model_cover.top + model_cover.height/2 - init_model_cover_bar_height/2

          var model_cover_bar = new fabric.Rect({
            fill: 'rgb(255,255,255)',
            strokeWidth: 0,
            top: init_model_cover_bar_top,
            left: init_model_cover_bar_left,
            width: init_model_cover_bar_width, 
            height: init_model_cover_bar_height, 
            hasControls: false,
            selectable: false,
          })

          // canvas.add(model_cover_bar_edge)
          canvas.add(model_cover_bar)


          var mdl_part_of_wsp_cover = new fabric.Rect({
            fill: 'rgb(128,128,128)',
            top: mdl_grid_top,
            left: mdl_grid_left, 
            width: trial.mdl_grid_dims[1] * grid_box_size, 
          ...
          var rcs_part_of_wsp_cover = new fabric.Rect({
            fill: 'rgb(128,128,128)',
            top: rsc_grid_top,
            left: rsc_grid_left,
            width: trial.rsc_grid_dims[1] * grid_box_size, 
          ...
          var wsp_part_of_wsp_cover = new fabric.Rect({
            fill: 'rgb(128,128,128)',
            top: wsp_grid_top,
            left: wsp_grid_left, 
            width: trial.mdl_grid_dims[1] * grid_box_size, 
          ...
          var wsp_cover_text = new fabric.Text('You made an error.\nClick and hold here\nto remove all covers.', { 
            left: wsp_grid_left + wsp_part_of_wsp_cover.width/2,
            originX: 'center',
            top: wsp_grid_top*1.17, 
            fontSize: (canvas_width/65),
            fontFamily: 'Arial',
            textAlign: 'center' 
          });
          ...
          var workspace_cover = new fabric.Group([wsp_part_of_wsp_cover, rcs_part_of_wsp_cover, mdl_part_of_wsp_cover, wsp_cover_text])
          ...
          // add state tracker
          workspace_cover.isLocked = false;
          
          canvas.add(workspace_cover)
          workspace_cover.setCoords()
          

          // declare timeout variable
          var workspace_timeout;


          // Add animations (workspace cover)
          var init_workspace_cover_bar_width = init_model_cover_bar_width;
          var init_workspace_cover_bar_left = wsp_grid_left + workspace_cover.width/2 - init_workspace_cover_bar_width/2; // centered on the workspace cover

          var init_workspace_cover_bar_left = wsp_grid_left + wsp_part_of_wsp_cover.width/2 - init_workspace_cover_bar_width/2; // centered on the workspace cover
          
          var init_workspace_cover_bar_height = init_model_cover_bar_height;
          var init_workspace_cover_bar_top = init_model_cover_bar_top;

          var workspace_cover_bar = new fabric.Rect({
            fill: 'rgb(255,255,255)',
            strokeWidth: 0,
            top: init_workspace_cover_bar_top,
            left: init_workspace_cover_bar_left,
            width: 0, 
            height: init_workspace_cover_bar_height, 
            hasControls: false,
            selectable: false,
          })

          canvas.add(workspace_cover_bar)
          
          // finnaly render all 
          canvas.renderAll();

          // Tack on mouse events:
          canvas.on('mouse:down', mouseDownOnCanvas);
          canvas.on('mouse:up', mouseUpOnCanvas);
          canvas.on('mouse:move', mouseMoveOnCanvas);

          function mouseDownOnCanvas(opts) {      

            let mousePos = [opts.e.layerX, opts.e.layerY]
            
            // model cover
            if ( posInRect(mousePos, model_cover) && ! workspace_cover.isLocked) { // if on model cover and workspace cover is open

              // start animation 
              model_cover_bar.animate('width', 0, {
                onChange: canvas.renderAll.bind(canvas),
                duration: trial.sampling_delay,
                easing: noEasing
              });

              // register event
              var event = {
                event: 'model cover is clicked on',
                timestamp: Math.round( performance.now() )
              }
              trial_events.push(event)

              // set timeout
              model_timeout = setTimeout( () => {
                // make transparent
                model_cover.opacity = 0;
                model_cover.setCoords()

                model_cover.isLocked = false;

                // register event
                var event = {
                  event: 'model is opened - due to mouse hold',
                  timestamp: Math.round( performance.now() )
                }
                trial_events.push(event)

                // cover workspace and resources
                workspace_cover_during_model_view.set({left:init_workspace_cover_during_model_view_left})
                workspace_cover_during_model_view.setCoords();

                // finnaly render all 
                canvas.renderAll();
    
              }, trial.sampling_delay)

            } 
            
              // workspace cover
            if (isInWorkspace (mousePos) && workspace_cover.isLocked) { // if on workspace cover
              
              // start animation 
              workspace_cover_bar.animate('width', 0, {
                onChange: canvas.renderAll.bind(canvas),
                duration: trial.error_delay,
                easing: noEasing
              });

              // register event
              var event = {
                event: 'workspace cover is clicked on',
                timestamp: Math.round( performance.now() )
              }
              trial_events.push(event)
              
              // set timeout
              workspace_timeout = setTimeout( () => {              
                workspace_cover.set({left : canvas_width});
                workspace_cover_bar.set({left : canvas_width}); // also remove model cover bar
                workspace_cover.isLocked = false;             
                workspace_cover.setCoords()

                // register event
                var event = {
                  event: 'workspace is opened - due to mouse hold',
                  timestamp: Math.round( performance.now() )
                }
                trial_events.push(event)
          ...
          function mouseUpOnCanvas(opts) {

            if (model_cover.isLocked) {
                // model coverr
              ...
              // make opaque
              model_cover.opacity = 1;
              model_cover.setCoords()

              // cancel model bar animation and set fill to max
              model_cover_bar.dispose()
              model_cover_bar.set({
                width: init_model_cover_bar_width
              })

              model_cover_bar.setCoords()

              // open workspace cover durung model view
              workspace_cover_during_model_view.set({left:canvas_width})
              workspace_cover_during_model_view.setCoords();


                // workspace cover
              if (workspace_cover.isLocked) {
                clearTimeout(workspace_timeout);

                // cancel workspace bar animation
                workspace_cover_bar.dispose()
                workspace_cover_bar.set({
                  width: init_workspace_cover_bar_width
                })

                workspace_cover_bar.setCoords()
          ...
          function mouseMoveOnCanvas(opts) {
            let mousePos = [opts.e.layerX, opts.e.layerY]


              // model cover
            if ( !posInRect(mousePos, model_cover) ) { // if NOT on model cover
              //remove timeout
              clearTimeout(model_timeout);
              
              // cancel animation and set fill to max
              ...
              if(!model_cover.isLocked) {
                var event = {
                  event: 'model is locked - due to mouse move',
                  timestamp: Math.round( performance.now() )
                }

                trial_events.push(event)       
                
                model_cover.isLocked = true;    
              }
              ...
              //workspace cover
            if ( ! isInWorkspace(mousePos) 
                  && workspace_cover.isLocked         ) { // if NOT on workspace cover and currently locked
              //remove timeout
              clearTimeout(workspace_timeout);
              ...
              // move into place
              workspace_cover.set({left : mdl_grid_left});
              workspace_cover.setCoords()
                ...
                if (isInBox(item_center_xy, wsp_box)) { // if item is dropped in a box
                  if (wsp_box.content == null){ // if box is empty
                    ...
                    if ( ( item.itemType == wsp_box.correct_content ) || (trial.allow_incorr_placement) ){ // if item in the corect box OR if we allow incorrect placement
                      ...
                      // ensure corectly placed item cannot be moved from it's correct location
                      ...
                      if (trial.enable_delays){
                        ...
                        // add workspace cover
                        workspace_cover.isLocked = true;
                        workspace_cover.set({left : mdl_grid_left});
                        
                        canvas.setActiveObject(workspace_cover) // make active to bring to front.
                        canvas.setActiveObject(placeholder_object) // workaround the bug that active objects are moveable
                        workspace_cover.setCoords();                      
                        
                        // add workspace cover bar
                        workspace_cover_bar.set({width : init_workspace_cover_bar_width, left : init_workspace_cover_bar_left});
                        workspace_cover_bar.setCoords(); 

                        // register event
                        var event = {
                          event: 'workspace is locked - due to error',
                          timestamp: Math.round( performance.now() )
                        }
                        trial_events.push(event) 
        ...
        function isInWorkspace (xy) {
          ...
          && (xy[0] <  workspace_grid[0].left + trial.mdl_grid_dims[1] * grid_box_size) 
          && (xy[1] >= workspace_grid[0].top)   // top  of upper-left most box
          && (xy[1] <  workspace_grid[0].top  + trial.mdl_grid_dims[0] * grid_box_size)
              )
        }
        ...
        ///////////////////////////////////////////////////////////////////////
        /* SECTION 5: jsPsych code to handle the end of a trial */
    
        var startTime = Math.round( performance.now() );

        // start event timestamp
        var event = {
          event: 'copying trial was started',
        ...
        if (trial.stimulus_duration !== null) {
          this.jsPsych.pluginAPI.setTimeout(() => {
              display_element.querySelector("#jspsych-canvas-copying-task-button").style.visibility = "hidden";
      ...
      ///////////////////////////////////////////////////////////////////////
      /* SECTION 6: return the plugin */

    CopyingTaskButtonPlugin.info = info;
  
    return CopyingTaskButtonPlugin;
  })(jsPsychModule);